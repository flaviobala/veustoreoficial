//app/api/vendas/route.ts (VERSÃO CORRIGIDA)
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 4. Converte a data para o formato correto
    const dataVendaFormatada = new Date(dataVenda);

    // 5. Inicia uma transação para garantir consistência
    const resultado = await prisma.$transaction(async (tx) => {
      ...
      const novaVenda = await tx.venda.create({
        data: {
          ...
          preco: precoTotal,
          cliente,
          dataVenda: dataVendaFormatada, // Usando a data formatada
          status,
        },
      });

      // 5.2 Atualiza o estoque do produto
      ...
      // 5.3 Registra entrada no caixa (apenas se venda for concluída)
      if (status === "Concluída") {
