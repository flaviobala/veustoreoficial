...
export async function PUT(req: NextRequest, context: Context) {
  try {
    ...
    if (!descricao || !tipo || !valor) {
      return NextResponse.json({ error: "Dados incompletos. Informe descrição, tipo e valor." }, { status: 400 });
    }
    ...
    const movimentoAtualizado = await prisma.caixa.update({
      where: { id },
      data: { descricao, tipo, valor: Number(valor) },
    });

    return NextResponse.json(movimentoAtualizado);
  } catch (error) {
    console.error("Erro no PUT /api/caixa/[id]:", error);
    return NextResponse.json({ error: "Erro interno ao atualizar movimento." }, { status: 500 });
  }
}
